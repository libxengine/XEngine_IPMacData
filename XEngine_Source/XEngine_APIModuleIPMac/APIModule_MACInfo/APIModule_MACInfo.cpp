...
bool CAPIModule_MACInfo::APIModule_MACInfo_Init(LPCXSTR lpszDBFile)
{
	...
	while (true)
	{
		...
		_tcsxncpy(st_MACInfo.tszMACPrefix, stl_ListField[0].c_str(), sizeof(st_MACInfo.tszMACPrefix) / sizeof(XCHAR) - 1);
		st_MACInfo.tszMACPrefix[sizeof(st_MACInfo.tszMACPrefix) / sizeof(XCHAR) - 1] = '\0';
		_tcsxncpy(st_MACInfo.tszVendorName, stl_ListField[1].c_str(), sizeof(st_MACInfo.tszVendorName) / sizeof(XCHAR) - 1);
		st_MACInfo.tszVendorName[sizeof(st_MACInfo.tszVendorName) / sizeof(XCHAR) - 1] = '\0';
		_tcsxncpy(st_MACInfo.tszBlockType, stl_ListField[3].c_str(), sizeof(st_MACInfo.tszBlockType) / sizeof(XCHAR) - 1);
		st_MACInfo.tszBlockType[sizeof(st_MACInfo.tszBlockType) / sizeof(XCHAR) - 1] = '\0';
		_tcsxncpy(st_MACInfo.tszUPTime, stl_ListField[4].c_str(), sizeof(st_MACInfo.tszUPTime) / sizeof(XCHAR) - 1);
		st_MACInfo.tszUPTime[sizeof(st_MACInfo.tszUPTime) / sizeof(XCHAR) - 1] = '\0';
...
void CAPIModule_MACInfo::APIModule_MACInfo_CSVParse(LPCXSTR lpszMSGBuffer, std::vector<xstring>* pStl_ListField)
{
	...
	while (*lpszMSGBuffer)
	{
		...
		{
			if (nIndex < (int)(sizeof(tszFieldStr) / sizeof(XCHAR) - 1))
			{
